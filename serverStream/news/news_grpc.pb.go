...
const (
	NewsService_GetNewsStream_FullMethodName = "/news.NewsService/GetNewsStream"
)
...
type NewsServiceClient interface {
	GetNewsStream(ctx context.Context, in *NewsRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[NewsResponse], error)
}
...
func (c *newsServiceClient) GetNewsStream(ctx context.Context, in *NewsRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[NewsResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &NewsService_ServiceDesc.Streams[0], NewsService_GetNewsStream_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[NewsRequest, NewsResponse]{ClientStream: stream}
...
type NewsService_GetNewsStreamClient = grpc.ServerStreamingClient[NewsResponse]

// NewsServiceServer is the server API for NewsService service.
// All implementations must embed UnimplementedNewsServiceServer
// for forward compatibility.
type NewsServiceServer interface {
	GetNewsStream(*NewsRequest, grpc.ServerStreamingServer[NewsResponse]) error
...
func (UnimplementedNewsServiceServer) GetNewsStream(*NewsRequest, grpc.ServerStreamingServer[NewsResponse]) error {
...
func _NewsService_GetNewsStream_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(NewsRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(NewsServiceServer).GetNewsStream(m, &grpc.GenericServerStream[NewsRequest, NewsResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type NewsService_GetNewsStreamServer = grpc.ServerStreamingServer[NewsResponse]
	...
	Streams: []grpc.StreamDesc{
		{
			...
			Handler:       _NewsService_GetNewsStream_Handler,
			ServerStreams: true,
		},
	},
